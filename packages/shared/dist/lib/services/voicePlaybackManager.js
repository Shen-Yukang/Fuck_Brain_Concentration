...
export class VoicePlaybackManager {
    ...
    // 开始TTS播放
    async startTTSPlayback(text, callbacks = {}) {
        ...
        try {
            ...
            const voiceError = error instanceof VoiceError
                ? error
                : new VoiceError(VoiceErrorType.TTS_PLAYBACK_FAILED, 'Failed to start TTS playback', error);
            this.handleError(voiceError);
            throw voiceError;
        }
    }
    // 开始Web Speech API播放
    async startWebSpeechPlayback(text, options = {}, callbacks = {}) {
        if (this.state.isPlaying) {
            await this.stop();
        }
        ...
        return new Promise((resolve, reject) => {
            try {
                ...
                utterance.onerror = event => {
                    const error = new VoiceError(VoiceErrorType.TTS_PLAYBACK_FAILED, `Web Speech error: ${event.error}`, new Error(event.error));
                ...
                const voiceError = new VoiceError(VoiceErrorType.TTS_PLAYBACK_FAILED, 'Failed to start Web Speech playback', error);
                this.handleError(voiceError);
                reject(voiceError);
            }
        });
    ...
    // 计算估算播放时长
    calculateEstimatedDuration(text) {
    ...
    // 安排播放结束检测（仅用于TTS）
    schedulePlaybackEnd(estimatedDuration) {
        if (this.endTimeout) {
            clearTimeout(this.endTimeout);
        }
        // 添加一些缓冲时间
